...
async def calculate_accuracy(max_stats, stats, full_combo=False, passed=False):
    ...
    current_maximum_base_score = sum(get_value_or_zero(key, max_stats) * base_scores.get(key, 0) for key in base_scores)
...
async def download_and_extract(url, file_to_extract, message):
    temp_zip_path = f'{file_to_extract}.zip'
            ...
            with open(temp_zip_path, 'wb') as f:
                ...
                async for chunk in response.content.iter_chunked(chunk_size):
                    ...
                    if current_progress // 10 != last_progress:
                        ...
                        progress_bar = '█' * (current_progress // 10)
                        progress_bar += '░' * (10 - (current_progress // 10))
                        embed_description = f"Downloading map...\n{progress_bar} {current_progress}%"

                        embed = discord.Embed(description=embed_description, colour=discord.Colour.orange())
        ...
        with zipfile.ZipFile(temp_zip_path, 'r') as zip_ref:
            for file_name in zip_ref.namelist():
                if file_name.endswith('.osu'):
                    extracted_path = zip_ref.extract(file_name, "data/osu_maps/")
                    new_path = await rename_file(extracted_path, file_name)
...
async def mod_math(mods, beatmap, settings):
    ...
    async def CalculateOD(OD, multiplier):
    ...
    if 'DT' in str(mods):
        ...
        beatmap.accuracy = await CalculateOD(beatmap.accuracy, speed)
